...
def Global_analyse(O, N_traj, N_step):
    ...
    L_analytic = acces_fnc.L(X2, Y2,u)                        #D'après les notes de Gleb
...
X2, Y2 = Global_analyse(OM,N_traj=N_traj, N_step = N_step)

r2_mean = np.mean(X2+Y2, axis=0)
X2_analy = acces_fnc.Var_analy(u, Tau_RT_x, Tau_RT_y, Tx, Ty)[0]
Y2_analy = acces_fnc.Var_analy(u, Tau_RT_x, Tau_RT_y, Tx, Ty)[1]
